...
namespace EntityFrameworkCore.SingleStore.Tests.ValueGeneration.Internal
{
    public class SingleStoreSequentialGuidValueGeneratorTest
    {
        private SingleStoreSequentialGuidValueGenerator GetGenerator(bool mysqlGuidFormatIsLittleEndianBinary16) {
            ...
            connectionSettings.SetupGet(x => x.GuidFormat).Returns(mysqlGuidFormatIsLittleEndianBinary16 ? SingleStoreGuidFormat.LittleEndianBinary16 : SingleStoreGuidFormat.None);

            var options = new Mock<SingleStoreOptions>();
            options.SetupGet(x => x.ConnectionSettings).Returns(connectionSettings.Object);

            return new SingleStoreSequentialGuidValueGenerator(options.Object);
        }

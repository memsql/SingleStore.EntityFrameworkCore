...
{
    public class SingleStoreUpdateSqlGeneratorTest : UpdateSqlGeneratorTestBase
    {
        protected override IUpdateSqlGenerator CreateSqlGenerator()
        {
            var options = GetOptions();
            return new SingleStoreUpdateSqlGenerator(
                new UpdateSqlGeneratorDependencies(
                    new SingleStoreSqlGenerationHelper(
        ...
        private static SingleStoreOptions GetOptions()
        {
            var optionsBuilder = new DbContextOptionsBuilder();

            var extension = new SingleStoreOptionsExtension()
                .WithServerVersion(AppConfig.ServerVersion);

            ((IDbContextOptionsBuilderInfrastructure)optionsBuilder).AddOrUpdateExtension(extension);

            var options = new SingleStoreOptions();

...
public class TPCManyToManyNoTrackingQuerySingleStoreTest : TPCManyToManyNoTrackingQueryRelationalTestBase<TPCManyToManyQuerySingleStoreFixture>
{
    public TPCManyToManyNoTrackingQuerySingleStoreTest(TPCManyToManyQuerySingleStoreFixture fixture, ITestOutputHelper testOutputHelper)
        : base(fixture)
    {
...
""");
    ...
    public override async Task Skip_navigation_select_many_average(bool async)
    {
        ...
        AssertSql(
$"""
SELECT AVG({SingleStoreTestHelpers.CastAsDouble(@"`t`.`Key1`")})
FROM `EntityTwos` AS `e`
INNER JOIN (
...
$"""
SELECT (
    SELECT AVG({SingleStoreTestHelpers.CastAsDouble(@"`e`.`Key1`")})
...
LEFT JOIN (
    ...
    INNER JOIN (
        SELECT `r`.`Id`, `r`.`Name`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"NULL", "signed")} AS `Number`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"NULL", "signed")} AS `IsGreen`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"'EntityRoot'")} AS `Discriminator`
        FROM `Roots` AS `r`
        UNION ALL
        SELECT `b`.`Id`, `b`.`Name`, `b`.`Number`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"NULL", "signed")} AS `IsGreen`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"'EntityBranch'")} AS `Discriminator`
        FROM `Branches` AS `b`
        UNION ALL
        SELECT `l`.`Id`, `l`.`Name`, `l`.`Number`, `l`.`IsGreen`, 'EntityLeaf' AS `Discriminator`
        FROM `Leaves` AS `l`
        UNION ALL
        SELECT `l0`.`Id`, `l0`.`Name`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"NULL", "signed")} AS `Number`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"NULL", "signed")} AS `IsGreen`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"'EntityLeaf2'")} AS `Discriminator`
        FROM `Leaf2s` AS `l0`
    ) AS `t` ON `e0`.`RootSkipSharedId` = `t`.`Id`
...
LEFT JOIN (
    ...
    FROM `EntityCompositeKeyEntityRoot` AS `e0`
    INNER JOIN (
        SELECT `r`.`Id`, `r`.`Name`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"NULL", "signed")} AS `Number`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"NULL", "signed")} AS `Slumber`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"NULL", "signed")} AS `IsGreen`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"NULL", "signed")} AS `IsBrown`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"'EntityRoot'")} AS `Discriminator`
        FROM `Roots` AS `r`
        UNION ALL
        SELECT `b`.`Id`, `b`.`Name`, `b`.`Number`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"NULL", "signed")} AS `Slumber`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"NULL", "signed")} AS `IsGreen`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"NULL", "signed")} AS `IsBrown`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"'EntityBranch'")} AS `Discriminator`
        FROM `Branches` AS `b`
        UNION ALL
        SELECT `l`.`Id`, `l`.`Name`, `l`.`Number`, NULL AS `Slumber`, `l`.`IsGreen`, NULL AS `IsBrown`, 'EntityLeaf' AS `Discriminator`
        FROM `Leaves` AS `l`
        UNION ALL
        SELECT `l0`.`Id`, `l0`.`Name`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"NULL", "signed")} AS `Number`, `l0`.`Slumber`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"NULL", "signed")} AS `IsGreen`, `l0`.`IsBrown`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"'EntityLeaf2'")} AS `Discriminator`
        FROM `Leaf2s` AS `l0`
    ) AS `t` ON `e0`.`RootSkipSharedId` = `t`.`Id`
) AS `t0` ON ((`e`.`Key1` = `t0`.`CompositeKeySkipSharedKey1`) AND (`e`.`Key2` = `t0`.`CompositeKeySkipSharedKey2`)) AND (`e`.`Key3` = `t0`.`CompositeKeySkipSharedKey3`)
ORDER BY `e`.`Key1`, `e`.`Key2`, `e`.`Key3`, `t0`.`RootSkipSharedId`, `t0`.`CompositeKeySkipSharedKey1`, `t0`.`CompositeKeySkipSharedKey2`, `t0`.`CompositeKeySkipSharedKey3`
...
$"""
SELECT (
    SELECT AVG({SingleStoreTestHelpers.CastAsDouble(@"`u1`.`Key1`")})
...
LEFT JOIN (
    ...
    INNER JOIN (
        ...
        UNION ALL
        SELECT `u3`.`Id`, `u3`.`Name`, `u3`.`Number`, `u3`.`IsGreen`, {SingleStoreTestHelpers.SingleStoreBug96947Workaround(@"'UnidirectionalEntityLeaf'")} AS `Discriminator`

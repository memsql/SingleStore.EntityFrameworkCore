...
{
    public partial class NorthwindDbFunctionsQuerySingleStoreTest
    {
        [ConditionalFact]
...
WHERE `o`.`Quantity` LIKE '%M%'");
        ...
        [ConditionalFact(Skip = "Operator LIKE does not support the standard ESCAPE clause in SingleStore")]
...
WHERE `o`.`OrderID` LIKE '!%' ESCAPE '!'");
        ...
        [ConditionalTheory(Skip = "Feature 'COLLATE in WHERE statements' is not supported by SingleStore")]
        public override Task Collate_case_insensitive(bool async)
        {
            return base.Collate_case_insensitive(async);
        }

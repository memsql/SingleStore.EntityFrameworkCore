...
namespace EntityFrameworkCore.SingleStore.FunctionalTests.Scaffolding
{
    public class SingleStoreDatabaseModelFactoryTest : IClassFixture<SingleStoreDatabaseModelFactoryTest.SingleStoreDatabaseModelFixture>
    {
        protected SingleStoreDatabaseModelFixture Fixture { get; }

        public SingleStoreDatabaseModelFactoryTest(SingleStoreDatabaseModelFixture fixture)
        {
            Fixture = fixture;
            ...
            string cleanupSql)
        {
            try
            {
                ...
                var databaseModelFactory = new SingleStoreDatabaseModelFactory(
                    ...
                    Fixture.ServiceProvider.GetService<ISingleStoreOptions>());
                    ...
                    if (AppConfig.ServerVersion.Supports.SpatialReferenceSystemRestrictedColumns)
                    {
                        Assert.Equal(
                            0, columns.Single(c => c.Name == "Location")
                                .FindAnnotation(SingleStoreAnnotationNames.SpatialReferenceSystemId)
                                ?.Value);
                    }
                    else
                    {
...
CREATE TABLE `IceCreams` (
    ...
    FULLTEXT USING VERSION 2 `IX_IceCreams_Name` (`Name`)
                ...
                dbModel =>
                {
                    ...
                    Assert.Equal("Name", index.Columns[0].Name, StringComparer.OrdinalIgnoreCase);
                    Assert.Equal(true, index.FindAnnotation(SingleStoreAnnotationNames.FullTextIndex)?.Value);
                },
                @"DROP TABLE IF EXISTS `IceCreams`;");
        }
...
CREATE FULLTEXT INDEX `IX_IceCreams_Name` ON `IceCreams` (`Name`) /*!50703 WITH PARSER `ngram` */;",
                ...
                dbModel =>
                {
                    ...
                    Assert.Equal("Name", index.Columns[0].Name, StringComparer.OrdinalIgnoreCase);
                    Assert.Equal(true, index.FindAnnotation(SingleStoreAnnotationNames.FullTextIndex)?.Value);
                    Assert.Equal("ngram", index.FindAnnotation(SingleStoreAnnotationNames.FullTextParser)?.Value);
                },
                @"DROP TABLE IF EXISTS `IceCreams`;");
        }
...
CREATE SPATIAL INDEX `IX_IceCreams_Location` ON `IceCreamShop` (`Location`);",
                ...
                dbModel =>
                {
                    ...
                    Assert.Equal(true, index.FindAnnotation(SingleStoreAnnotationNames.SpatialIndex)?.Value);
                },
                @"DROP TABLE IF EXISTS `IceCreamShop`;");
        }
...
DROP TABLE IF EXISTS PrincipalTable;");
        ...
        public class SingleStoreDatabaseModelFixture : SharedStoreFixtureBase<PoolableDbContext>
        {
            protected override string StoreName { get; } = nameof(SingleStoreDatabaseModelFactoryTest);
            protected override ITestStoreFactory TestStoreFactory => SingleStoreTestStoreFactory.Instance;
            public new SingleStoreTestStore TestStore => (SingleStoreTestStore)base.TestStore;

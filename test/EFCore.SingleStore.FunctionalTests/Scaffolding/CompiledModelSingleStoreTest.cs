...
public class CompiledModelSingleStoreTest : CompiledModelRelationalTestBase
{
    ...
    public override Task Tpc_Sprocs()
    {
        ...
        return Test(
            ...
            addServices: s => s.AddSingleton<IModelValidator, StoredProcedureValidationExceptionIgnoringSingleStoreModelValidator>());
    }
    ...
    {
        Assert.Equal(
            SingleStoreStrings.Ef7CoreJsonMappingNotSupported,
    ...
    protected override TestHelpers TestHelpers => SingleStoreTestHelpers.Instance;
    ...
    protected override void AddDesignTimeServices(IServiceCollection services)
        => new SingleStoreNetTopologySuiteDesignTimeServices().ConfigureDesignTimeServices(services);

    protected override BuildSource AddReferences(BuildSource build, [CallerFilePath] string filePath = "")
    {
        base.AddReferences(build);
        build.References.Add(BuildReference.ByName("EntityFrameworkCore.SingleStore"));
        build.References.Add(BuildReference.ByName("EntityFrameworkCore.SingleStore.NetTopologySuite"));
    ...
    // validation to allow the unsupported stored procedure features.
    // We do the latter here.
    public class StoredProcedureValidationExceptionIgnoringSingleStoreModelValidator(
        ModelValidatorDependencies dependencies,
        RelationalModelValidatorDependencies relationalDependencies)
        : SingleStoreModelValidator(dependencies, relationalDependencies)
    {
        protected override void ValidateStoredProcedures(IModel model, IDiagnosticsLogger<DbLoggerCategory.Model.Validation> logger)
        {
            ...
            catch (InvalidOperationException e) when (Regex.IsMatch(e.Message, Regex.Escape(SingleStoreStrings.StoredProcedureResultColumnsNotSupported("::::", "::::")).Replace("::::", ".*")) ||
                                                      Regex.IsMatch(e.Message, Regex.Escape(SingleStoreStrings.StoredProcedureReturnValueNotSupported("::::", "::::")).Replace("::::", ".*")))
            {
            }
        }
    }

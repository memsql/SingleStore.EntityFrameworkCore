...
namespace EntityFrameworkCore.SingleStore.Query.ExpressionVisitors.Internal
{
    public class SingleStoreSqlTranslatingExpressionVisitor : RelationalSqlTranslatingExpressionVisitor
    {
        ...
        protected override Expression VisitUnary(UnaryExpression unaryExpression)
        {
            if (unaryExpression.NodeType == ExpressionType.ArrayLength)
            {
                ...
                if (sqlOperand.Type == typeof(byte[]) &&
                    (sqlOperand.TypeMapping is null or SingleStoreByteArrayTypeMapping))
                {
                ...
                unaryExpression.NodeType == ExpressionType.ConvertChecked)
            {
                ...
                if (visitedOperand is SingleStoreJsonTraversalExpression traversal)
                {
            ...
            if (binaryExpression.NodeType == ExpressionType.Subtract &&
                ...
                (/*subtractLeftVisited.Type == typeof(DateTime) && subtractRightVisited.Type == typeof(DateTime) ||
                 subtractLeftVisited.Type == typeof(DateTimeOffset) && subtractRightVisited.Type == typeof(DateTimeOffset) ||*/
                    (subtractLeftVisited.Type == typeof(TimeOnly) || subtractLeftVisited.Type == typeof(TimeSpan))
                    && (subtractRightVisited.Type == typeof(TimeOnly) || subtractRightVisited.Type == typeof(TimeSpan)))
               )
            {
                if (subtractRightVisited is SqlConstantExpression sqlConstantExpression)
                {
                    var right = new TimeSpan(0);

                    if (sqlConstantExpression.Value is TimeOnly timeonly)
                    {
                        right = new TimeSpan(-timeonly.Ticks);
                    }

                    if (sqlConstantExpression.Value is TimeSpan timespan)
                    {
                        right = new TimeSpan(-timespan.Ticks);
                    }

                    return _sqlExpressionFactory.Add(subtractLeftVisited,
                        _sqlExpressionFactory.Fragment("\"" + right.ToString() + "\""));
                }
        ...
        protected override Expression VisitMethodCall(MethodCallExpression methodCallExpression)
        {
            ...
            {
                var message = SingleStoreStrings.QueryUnableToTranslateMethodWithStringComparison(

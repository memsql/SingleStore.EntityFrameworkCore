...
{
    public class SingleStoreAnnotationProvider : RelationalAnnotationProvider
    {
        [NotNull] private readonly ISingleStoreOptions _options;

        public SingleStoreAnnotationProvider(
        ...
        public override IEnumerable<IAnnotation> For(IRelationalModel model, bool designTime)
        {
            ...
            // handled Collation property in SingleStoreMigrationsModelDiffer.

            // Handle other annotations (including the delegation annotations).
            foreach (var annotation in model.Model.GetAnnotations()
                .Where(a => a.Name is SingleStoreAnnotationNames.CharSetDelegation or
                                      SingleStoreAnnotationNames.CollationDelegation))
            {
        ...
        public override IEnumerable<IAnnotation> For(ITable table, bool designTime)
        {
            ...
            foreach (var annotation in entityType.GetAnnotations()
                .Where(a => a.Name is SingleStoreAnnotationNames.CharSetDelegation or
                                      SingleStoreAnnotationNames.CollationDelegation or
                                      SingleStoreAnnotationNames.StoreOptions))
            {
                yield return annotation;
            }
        }
        ...
        public override IEnumerable<IAnnotation> For(IColumn column, bool designTime)
        {
                ...
                .FirstOrDefault(p => p.GetValueGenerationStrategy(table) == SingleStoreValueGenerationStrategy.IdentityColumn) is IProperty identityProperty)
            {
                ...
                p => p.GetValueGenerationStrategy(table) == SingleStoreValueGenerationStrategy.ComputedColumn) is IProperty computedProperty)
            {
            ...
            {
                yield return new Annotation(
                    SingleStoreAnnotationNames.SpatialReferenceSystemId,
                    srid);
            }
        }
        ...
        protected virtual string GetActualPropertyCollation(IProperty[] properties, DelegationModes currentLevel)
        {
            ...
            // We have been using the `SingleStore:Collation` annotation before EF Core added collation support.
            // Our `SingleStorePropertyExtensions.GetSingleStoreLegacyCollation()` method handles the legacy case, so we explicitly

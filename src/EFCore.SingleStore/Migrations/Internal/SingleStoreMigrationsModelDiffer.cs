...
{
    public class SingleStoreMigrationsModelDiffer : MigrationsModelDiffer
    {
        // CHECK: Might be a good use case for runtime annotations.
        protected static class InternalLocalAnnotationNames
        {
            public const string InternalLocalPrefix = SingleStoreAnnotationNames.Prefix + "Internal:MigrationsModelDiffer:";
        ...
        protected override IEnumerable<MigrationOperation> Add(IColumn target, DiffContext diffContext, bool inline = false)
        {
            if (!inline)
            {
                foreach (var propertyMapping in target.PropertyMappings)
                {
                    ...
                    {
                        var valueGenerationStrategy = SingleStoreValueGenerationStrategyCompatibility.GetValueGenerationStrategy(
            ...
            IEnumerable<MigrationOperation> migrationOperations)
        {
            foreach (var operation in migrationOperations)
            {
                ...
                if (operation is not AlterColumnOperation alterColumnOperation ||
                    alterColumnOperation.OldColumn[SingleStoreAnnotationNames.CharSet] is not string oldColumnCharSet ||
                    alterColumnOperation[SingleStoreAnnotationNames.CharSet] is not string newColumnCharSet ||
            ...
            where TOperation : MigrationOperation
        {
            ...
            if (operation[SingleStoreAnnotationNames.CollationDelegation] is DelegationModes databaseCollationDelegation)
            {
                // Don't leak the CollationDelegation annotation to the MigrationOperation.
                operation[SingleStoreAnnotationNames.CollationDelegation] = null;
        ...
        private static void HandleCharSetDelegation(MigrationOperation operation, DelegationModes delegationModes)
        {
            ...
            if (operation[SingleStoreAnnotationNames.CharSetDelegation] is DelegationModes charSetDelegation)
            {
                // Don't leak the CharSetDelegation annotation to the MigrationOperation.
                operation[SingleStoreAnnotationNames.CharSetDelegation] = null;

                if (!charSetDelegation.HasFlag(delegationModes))
                {
                    operation[SingleStoreAnnotationNames.CharSet] = null;
                }
            }
        }

...
using EntityFrameworkCore.SingleStore.Extensions;
...
{
    public class SingleStoreDatabaseModelFactory : DatabaseModelFactory
    {
        private readonly IDiagnosticsLogger<DbLoggerCategory.Scaffolding> _logger;
        private readonly IRelationalTypeMappingSource _typeMappingSource;
        private readonly ISingleStoreOptions _options;

        protected virtual SingleStoreScaffoldingConnectionSettings Settings { get; set; }

        public SingleStoreDatabaseModelFactory(
            ...
            [NotNull] ISingleStoreOptions options)
        {
            ...
            _options = options;
            Settings = new SingleStoreScaffoldingConnectionSettings(string.Empty);
        }
        ...
        public override DatabaseModel Create(string connectionString, DatabaseModelFactoryOptions options)
        {
            ...
            Settings = new SingleStoreScaffoldingConnectionSettings(connectionString);

            using var connection = new SingleStoreConnection(Settings.GetProviderCompatibleConnectionString());
        ...
        public override DatabaseModel Create(DbConnection connection, DatabaseModelFactoryOptions options)
        {
            ...
            SetupSingleStoreOptions(connection);
        ...
        protected virtual void SetupSingleStoreOptions(DbConnection connection)
        {
            // Set the actual server version from the open connection here, so we can
            // access it from ISingleStoreOptions later when generating the code for the
            // `UseSingleStore()` call.

            if (Equals(_options, new SingleStoreOptions()))
            {
                _options.Initialize(
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                using (var reader = command.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        databaseModel[SingleStoreAnnotationNames.CharSet] = Settings.CharSet
                            ? defaultCharSet
                            : null;
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            if (generation is not null)
                            {
                                ...
                                generation = _options.ServerVersion.Supports.SingleStoreBug104294Workaround
                                    ? generation.Replace(@"\'", @"'")
                                    : generation;
                            }
                            ...
                            if (defaultValue != null)
                            {
                                ...
                                defaultValue = _options.ServerVersion.Supports.AlternativeDefaultExpression
                                    ? ConvertDefaultValueFromMariaDbToSingleStore(defaultValue, out isDefaultValueExpression)
                                    : defaultValue;
            ...
            foreach (var table in tables)
            {
                            ...
                            try
                            {
                                ...
                                if (hasPrefixLengths)
                                {
                                    if (index.Columns.Count <= 0)
                                    {
                                        ...
                                        index[SingleStoreAnnotationNames.IndexPrefixLength] = prefixLengths;
                                    }
                                    else
                                    {
                                        // Use no prefix length at all or the highest prefix length for a given column
                                        // from all indexes with the same set of columns.
                                        var existingPrefixLengths = (int[])index[SingleStoreAnnotationNames.IndexPrefixLength];
                                        ...
                                        index[SingleStoreAnnotationNames.IndexPrefixLength] = newPrefixLengths.Any(p => p > 0)
                                            ? newPrefixLengths
                                            : null;
                                    }
                                }
                                else
                                {
                                    ...
                                    index[SingleStoreAnnotationNames.IndexPrefixLength] = null;
                                }
                                ...
                                {
                                    index[SingleStoreAnnotationNames.SpatialIndex] = true;
                                }

                                if (string.Equals(indexType, "fulltext", StringComparison.OrdinalIgnoreCase))
                                {
                                    index[SingleStoreAnnotationNames.FullTextIndex] = true;
                                }
                ...
                if (fullTextIndexes.Any())
                {
                    ...
                    foreach (var fullTextIndex in fullTextIndexes)
                    {
                        ...
                        {
                            fullTextIndex[SingleStoreAnnotationNames.FullTextParser] = fullTextParser;
                        }
                    }
                }
            }
